...
@section('title', 'Editing ' . $result->student->name . '\'s Result')
@section('header')
    Editing <i><b> {{ $result->student->name }}'s</b></i> Result
@endsection
...
@section('content')
    <form method="post" action="{{ route('edit-result', $result->id) }}">
        @csrf
        ...
        <div class="form-floating mb-3">
            <input disabled class="form-control" value="{{ ucfirst($result->student->gender) }}">
            <label>Gender</label>
        </div>

        <div class="form-floating mb-3">
            <input disabled class="form-control" value="{{ $result->schoolClass->name }}">
            <label>Class</label>
        </div>
        ...
        <div class="form-floating mb-3">
            <input disabled name="session" class="form-control" value="{{ $result->session->name }}">
            <label>Session</label>
        </div>


        <div class="form-floating mb-3">
            <input disabled name="session" class="form-control" value="{{ $result->term->name }}">
            <label>Term</label>
        </div>
        ...
        <div id="extra_subjects" class="row mb-4 g-3">
            ...
            @endphp
            @for ($i = 0; $i < $arranged_subjects_count; ++$i)
                <div class="col-sm-12 col-md-6 col-lg-6 col-xl-6 subject{{ $arranged_subjects[$i]->id }}">
                    <div class="border mb-4 rounded border-dark shadow-sm py-4 px-3">
                        <a type="button" data-id="{{ $arranged_subjects[$i]->id }}" class="text-decoration-none text-vermillion subject-remover float-end"><span class="text-10"><i class="fa-light fa-times-circle"></i> Remove</span></a>

                        <div class="form-group">
                            <div class="mb-2"><b class="text-underline">{{ $arranged_subjects[$i]->name }}</b></div>
                            <input type="hidden" name="subject_id[]" value="{{ $arranged_subjects[$i]->id }}">

                            <div class="form-floating mb-3">

                                <input type="number" max="40" name="cas[]" value="{{ $scores[$i][0] }}"
    oninput="pairAndValue()"
                            ...
                            <div class="form-floating mb-3">
                                ...
                                <input
                                oninput="pairAndValue()"
                                type="number" max="60" name="exams[]" value="{{ $scores[$i][1] }}"
            ...
            @endfor
                ...
                @if($not_subjects->count())

            @foreach($not_subjects as $not_subject)
        <span type="button" name="{{ $not_subject->subject->name }}" onclick="addSubject(this)" id="{{ $not_subject->subject_id }}" class="badge bg-primary">{{ $not_subject->subject->name }}</span>
        ...
        @endforeach
            ...
            <div class="form-floating mb-5">
                ...
                <textarea style="height: 100px" placeholder="Teacher's Comment" id="comment" name="comment" rows="7"
                    class="form-control">{{ $result->comment }}</textarea>
        ...
        <input type="hidden" name="rating_id[]" value="{{ $rating_ids[$i] }}">

        <select required name="rating[]" class="form-select">
        <option hidden value="">-- SELECT --</option>
        <option {{ $ratings[$rating_ids[$i]] == "A" ? "selected": ""}} value="A">A</option>
        <option {{ $ratings[$rating_ids[$i]] == "B" ? "selected": ""}} value="B">B</option>
        <option {{ $ratings[$rating_ids[$i]] == "C" ? "selected": ""}} value="C">C</option>
        <option {{ $ratings[$rating_ids[$i]] == "D" ? "selected": ""}} value="D">D</option>
        <option {{ $ratings[$rating_ids[$i]] == "E" ? "selected": ""}} value="E">E</option>

        </select>
        <label>{{ $rating_names[$i] }}</label>
        </div>
      </div>
      @endfor
